...
const Round5 = () => {
    ...
    const [showShowRound5Score, setShowShowRound5Score] = useState(false)
    const [eachPlayerDone, setEachPlayerDone] = useState(false);
    const [everyoneScored, setEveryOneScored] = useState(0)
    const [readyForRound6, setReadyForRound6] = useState(false)
    ...
    async function addRoundCollection() {
        try {
            ...
            let round5CollectionRef = collection(userDocRef, 'round 5');
            let userRound5DocRef = doc(round5CollectionRef, userEmail);
    ...
    async function findEachSubmission() {
        ...
        try {
            ...
            for (const doc of trackingOnlinePlayersCol.docs) {
                const round5Subcollection = await getDocs(collection(doc.ref, 'round 5'));
                let eachObjName = doc.id;
                for (const round5Doc of round5Subcollection.docs) {
                    const round5Data = round5Doc.data();
                    for (let j = 0; j < playerInfo.length; j++) {
                        ...
                        if (playerEmail === eachObjName) {
                            ...
                            const playerSong = round5Data.song;
                            const playerArtist = round5Data.artist;
                            const playerURL = round5Data.url;
 ...
 const newParticipantsNumber = participantsNumber - 1;
    ...
    async function scoreModal() {
        ...
        if (selectedScore) {
            setSelectedScore(0);
            try {
                const thePath = `${gameCode}/onlinePlayers/trackingOnlinePlayers/${sortedSubmissions[theOrder].emailId}/round 5/${sortedSubmissions[theOrder].emailId}`
...
const theModal = (
    ...
    async function updateRound5Done() {
        if (endOfRound) {
            const path = `${gameCode}/onlinePlayers/trackingOnlinePlayers/${userEmail}/round 5/${userEmail}`;
    ...
    const checkDoneStatus = async () => {
        ...
        const snapshotPromises = sortedSubmissions.map(async (submission, i) => {
            ...
            const eachPath = `${gameCode}/onlinePlayers/trackingOnlinePlayers/${theEmail}/round 5/${theEmail}`;
            return new Promise((resolve, reject) => {
                const unsubscribe = onSnapshot(doc(firestore, eachPath), (doc) => {
                    if (doc.exists()) {
                        const currentData = doc.data();
                        if (currentData && currentData.round5Done === true) {
                            doneArray.push(i);
                        }
                        resolve();
                    } else {
    ...
    async function showRound5Scores() {
        let everyoneScored = []
        for (let i = 0; i < sortedSubmissions.length; i++) {
            const theEmail = sortedSubmissions[i].emailId
            const theSong = sortedSubmissions[i].song
            const eachPath = `${gameCode}/onlinePlayers/trackingOnlinePlayers/${theEmail}/round 5/${theEmail}/scores/${theEmail}`
            try {
        ...
        setReadyForRound6(true)
        setEveryOneScored(everyoneScored.length)
        setShowShowRound5Score(false)
    ...
    useEffect(() => {
        eachPlayerDone ? setShowShowRound5Score(true) : setShowShowRound5Score(false)
    }), [eachPlayerDone];
...
<Text style={{ fontSize: 20 }}>Round 5</Text>
<TouchableOpacity
style={{
backgroundColor: 'red',
padding: 5,
...
<Text style={{ color: 'white' }}>Leave Game</Text>
          ...
          <View style={{ flexGrow: 1,  backgroundColor: '#FFD699' }}>
                            ...
                            {showRoundChoicesButton &&
                                       ...
                                       >
                                           <Text style={{ color: 'white', fontSize: 24 }} >Reveal and score Round 5 choices</Text>
                                       </TouchableOpacity>
                                      </View>
                            }
                            {theBoxes}
                            {theModal}
                            {theFunModal}
                            {endOfRound && readyForRound6 === false && showShowRound5Score === false &&
                            ...
                            {readyForRound6 && everyoneScored === boxNumbers &&
                                <View style={{ alignItems: 'center' }}>
                                    <Text style={{ fontSize: 24, fontWeight: 'bold' }}>Round 5 is over!</Text>
                                    <TouchableOpacity onPress={() => goToRound6()} style={{ backgroundColor: 'red', padding: 15, borderRadius: 5 }}>
