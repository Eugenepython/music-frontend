...
import { Modal, ScrollView, SafeAreaView, View, Text, TextInput, Button, StyleSheet, TouchableOpacity, Linking, BackHandler, Touchable, Image  } from 'react-native';
...
const Round3 = () => {
    ...
    const [showShowRound3Score, setShowShowRound3Score] = useState(false)
    const [eachPlayerDone, setEachPlayerDone] = useState(false);
    const [everyoneScored, setEveryOneScored] = useState(0)
    const [readyForRound4, setReadyForRound4] = useState(false)
    ...
    async function addRoundCollection() {
        try {
            ...
            let round3CollectionRef = collection(userDocRef, 'round 3');
            let userRound3DocRef = doc(round3CollectionRef, userEmail);
    ...
    async function findEachSubmission() {
        ...
        try {
            ...
            for (const doc of trackingOnlinePlayersCol.docs) {
                const round3Subcollection = await getDocs(collection(doc.ref, 'round 3'));
                let eachObjName = doc.id;
                for (const round3Doc of round3Subcollection.docs) {
                    const round3Data = round3Doc.data();
                    for (let j = 0; j < playerInfo.length; j++) {
                        ...
                        if (playerEmail === eachObjName) {
                            ...
                            const playerSong = round3Data.song;
                            const playerArtist = round3Data.artist;
                            const playerURL = round3Data.url;
 ...
 const newParticipantsNumber = participantsNumber - 1;
    ...
    async function scoreModal() {
        ...
        if (selectedScore) {
            setSelectedScore(0);
            try {
                const thePath = `${gameCode}/onlinePlayers/trackingOnlinePlayers/${sortedSubmissions[theOrder].emailId}/round 3/${sortedSubmissions[theOrder].emailId}`
...
const theModal = (
    ...
    async function updateRound3Done() {
        if (endOfRound) {
            const path = `${gameCode}/onlinePlayers/trackingOnlinePlayers/${userEmail}/round 3/${userEmail}`;
    ...
    const checkDoneStatus = async () => {
        ...
        const snapshotPromises = sortedSubmissions.map(async (submission, i) => {
            ...
            const eachPath = `${gameCode}/onlinePlayers/trackingOnlinePlayers/${theEmail}/round 3/${theEmail}`;
            return new Promise((resolve, reject) => {
                const unsubscribe = onSnapshot(doc(firestore, eachPath), (doc) => {
                    if (doc.exists()) {
                        const currentData = doc.data();
                        if (currentData && currentData.round3Done === true) {
                            doneArray.push(i);
                        }
                        resolve();
                    } else {
    ...
    async function showRound3Scores() {
        let everyoneScored = []
        for (let i = 0; i < sortedSubmissions.length; i++) {
            const theEmail = sortedSubmissions[i].emailId
            const theSong = sortedSubmissions[i].song
            const eachPath = `${gameCode}/onlinePlayers/trackingOnlinePlayers/${theEmail}/round 3/${theEmail}/scores/${theEmail}`
            try {
        ...
        setReadyForRound4(true)
        setEveryOneScored(everyoneScored.length)
        setShowShowRound3Score(false)
        ...
        try {
            ...
            if (docSnapshot.exists()) {
                ...
                if (maxNo === startersNo) {
                    ...
                    navigation.navigate("Go to Round 4", {
                        screen: "NestedRound4",
                        params: {}
                    });
                } else {
                    console.log("this guy was not the first to go to round 4")
                    navigation.navigate("Go to Round 4", {
                        screen: "NestedRound4",
                        params: {}
                    });
    ...
    useEffect(() => {
        eachPlayerDone ? setShowShowRound3Score(true) : setShowShowRound3Score(false)
    }), [eachPlayerDone];
    ...
    const theMissing = theMissingPlayers ? (
        ...
        <View key={0}>
            <Text>Waiting for everyone to finish Round 3</Text>
        </View>
    );
...
<Text style={{ fontSize: 20 }}>Round 3</Text>
<TouchableOpacity
style={{
backgroundColor: 'red',
padding: 5,
                   ...
                   <Text style = {{color: 'white', textAlign: 'center'}}>YOUTUBE URL</Text>
                            ...
                            {title && artist && URL && (
                                <View style={{ marginTop: 10 }}>
                                    ...
                                    >
                                        <Text style={{  color: 'white', padding: 10, borderRadius: 5, textAlign: 'center' }}>Submit</Text>
                                    </TouchableOpacity>
                                </View>
                            )}
                        </View>
          ...
          <View style={{ flexGrow: 1,  backgroundColor: '#FFD699' }}>
                          <View style={styles.titleContainer}>
                            ...
                            {showRoundChoicesButton &&
                                        ...
                                        >
                                            <Text style={{ color: 'white', fontSize: 24 }} >Reveal and score Round 3 choices</Text>
                                        </TouchableOpacity>
                                       </View>
                            }
                            {theBoxes}
                            {theModal}
                            {theFunModal}
                            {endOfRound && readyForRound4 === false && showShowRound3Score === false &&
                            ...
                            {readyForRound4 && everyoneScored === boxNumbers &&
                                <View style={{ alignItems: 'center' }}>
                                    <Text style={{ fontSize: 24, fontWeight: 'bold' }}>Round 3 is over!</Text>


                                    {roundsNumber > 3 &&
                                        <Button title="Go to Round 4" onPress={() => goToRound4()} />
                                    }
                                    {roundsNumber === 3 &&
                                          <TouchableOpacity onPress={() => goToScorePage()} style={{ backgroundColor: 'red', padding: 15, borderRadius: 5 }}>
                                          <Text style={{ color: 'white', textAlign: 'center', fontSize : 24}}>The game is over, check out the scores!</Text>
                                        </TouchableOpacity>
